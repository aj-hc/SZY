...
//打开住院号录入框
function openzhuyuanhaoiputbox() {
    $('#zhuyuanhaoiputbox').dialog('open');
    $('#zhuyuanhaoiputbox input.validatebox-text').focus();
}
//打开门诊号录入框
function openmenzhenhaoiputbox() {
    $('#menzhenhaoiputbox').dialog('open');
    $('#menzhenhaoiputbox input.validatebox-text').focus();
}
//条码框按钮回车事件
$(function () {
    $("input", $("#zhuyuanhaobox").next("span")).keydown(function (e) {
        if (e.keyCode == 13) {
            var code = $('#zhuyuanhaobox').textbox('getValue');
            if ($.trim(code)) {
                getdatabyzhuyuanhao(code);
                var code = $('#zhuyuanhaobox').textbox('clear');
            }
            var code = $('#zhuyuanhaobox').textbox('clear');
        }
    });
    $("input", $("#menzhenhaobox").next("span")).keydown(function (e) {
        if (e.keyCode == 13) {
            var code = $('#menzhenhaobox').textbox('getValue');
            if ($.trim(code)) {
                getdatabymenzhenhao(code);
                var code = $('#menzhenhaobox').textbox('clear');
            }
            var code = $('#menzhenhaobox').textbox('clear');
        }
    });
...
//根据住院号查询数据
function zhuyuanhao(code) {
    accept();
    $.ajax({
        url: "GetData.ashx",
        type: "post",
        data: { "GetType": "getdatabyMzhzyh", "Mzzybz": 1, "Mzhzyh": code },
        dataType: "text",
        success: function (data) {
            if (data == "") {
                alert("无数据");
...
//根据门诊号查询数据
function menzhenhao(code) {
    accept();
    $.ajax({
        url: "GetData.ashx",
        type: "post",
        data: { "GetType": "getdatabyMzhzyh", "Mzzybz": 0, "Mzhzyh": code },
        dataType: "text",
        success: function (data) {
            if (data == "") {
                alert("无数据");
