        ...
        private Model.QueryRecoder ContrastQueryRecoderModel(Model.QueryRecoder newModel, Model.QueryRecoder oldModel)
        {
                ...
                if ((DateTime.Now - oldModel.AddDate).Days > queryDateInterval)
                {
                    ...
                    try
                    {
                        ...
                        if (updateResult)
                        {
                            ...
                            this.RequestStr = CreatRequestStr(oldModel.Code, oldModel.AddDate.AddDays(queryDateInterval).ToString("yyyy-MM-dd"));
                        }
                    }
                    catch (Exception ex)
                    {
            ...
            else if (newModel != null && oldModel != null)
            {
                ...
                else if (dateDifWithOldAddDateAndDateNow >= 2 * queryDateInterval)
                {
                    ...
                    resultModel = oldModel;
                    this.RequestStr = CreatRequestStr(Code, oldModel.AddDate.AddDays(queryDateInterval).ToString("yyyy-MM-dd"));
                }
                ...
                else
                {
                    ...
                    //没超时，还在时间范围内的
                    oldModel.LastQueryDate = DateTime.Now;
                    this.RequestStr = CreatRequestStr(strCode, DateTime.Now.ToString("yyyy-MM-dd"));
                    resultModel = oldModel;
                }
            }
            return resultModel;
